...
struct WebDAVSyncTransport: SyncTransport {
    ...
    private var rootURL: URL {
        baseURL.appendingPathComponent(".clipth-sync-v1", isDirectory: true)
    }
    ...
    private func send(
        ...
        request.setValue("Clipth/1", forHTTPHeaderField: "User-Agent")
...
struct S3SyncTransport: SyncTransport {
    ...
    var displayLocation: String {
        ...
        return "s3://\(configuration.bucket)/\(prefix.isEmpty ? "" : "\(prefix)/").clipth-sync-v1"
    }
    ...
    private var rootKey: String {
        [normalizedPrefix, ".clipth-sync-v1"].filter { !$0.isEmpty }.joined(separator: "/")
    }
