...
class ExportService {
    ...
    private static let exportSchemaVersion = 2
    private static let minimumReadableSchemaVersion = 1
    private static let exportFormat = "clipth.clipboard-export"
    ...
    func exportToJSON(items: [ClipboardItem], filter: ExportFilter, completion: @escaping (Result<URL, Error>?) -> Void) {
        ...
        panel.nameFieldStringValue = "clipth_export_\(stamp).json"
    ...
    /// Import JSON files produced by Clipth. The decoder is deliberately
    /// permissive: older files lack schema metadata, while future files may add
    /// fields that this build can safely ignore.
    @MainActor
    func importFromJSON(context: ModelContext, completion: @escaping (Result<ClipboardJSONImportResult, Error>?) -> Void) {
        let panel = NSOpenPanel()
        panel.canChooseFiles = true
        panel.canChooseDirectories = false
        panel.allowsMultipleSelection = false
        panel.allowedContentTypes = [.json]
        panel.title = L("import.openPanel.title")
        panel.prompt = L("import.openPanel.prompt")
        ...
        panel.begin { response in
            ...
            do {
                let result = try Self.importPayload(from: url, into: context)
    ...
    private static func makePayload(items: [ClipboardItem], includeImageData: Bool) -> ExportPayload {
        ...
        let mapped = items.map { item -> ExportItemDTO in
            ...
            let imagePayload = includeImageData
                ? ImagePayloadStore.payload(for: ImagePayloadStore.reference(for: item))
                : nil
            return ExportItemDTO(
                ...
                groupIDs: item.groupIDs.isEmpty ? nil : item.groupIDs.map(\.uuidString),
                imageUTI: imagePayload?.uti ?? item.imageUTI,
                imageByteCount: imagePayload?.byteCount ?? item.imageByteCount,
                imagePixelWidth: imagePayload?.pixelWidth ?? item.imagePixelWidth,
                imagePixelHeight: imagePayload?.pixelHeight ?? item.imagePixelHeight,
        ...
        return ExportPayload(
            ...
            minimumReadableSchemaVersion: minimumReadableSchemaVersion,
            format: exportFormat,
            applicationID: "com.wavever.clipth",
            applicationName: "Clipth",
            bundleIdentifier: Bundle.main.bundleIdentifier ?? "com.wavever.clipth",
            exportedAt: Date(),
            count: mapped.count,
            items: mapped
        )
    ...
    private static func importPayload(from url: URL, into context: ModelContext) throws -> ClipboardJSONImportResult {
        let data = try Data(contentsOf: url)
        let decoder = JSONDecoder()
        decoder.dateDecodingStrategy = .iso8601
        let payload = try decoder.decode(ExportPayload.self, from: data)
        let requiredReaderVersion = payload.minimumReadableSchemaVersion ?? 1
        guard requiredReaderVersion <= exportSchemaVersion else {
            throw ClipboardJSONImportError.newerSchema(requiredReaderVersion)
        }

        var descriptor = FetchDescriptor<ClipboardItem>()
        descriptor.propertiesToFetch = [\.id]
        let existingIDs = (try? context.fetch(descriptor).map(\.id)) ?? []
        var knownIDs = Set(existingIDs)
        ...
        for dto in payload.items {
            guard let item = dto.makeClipboardItem() else {
                failed += 1
                continue
            }
        ...
        return ClipboardJSONImportResult(
            imported: imported,
            skipped: skipped,
            failed: failed,
            sourceSchemaVersion: payload.schemaVersion ?? 1
        )
    }
...
struct ClipboardJSONImportResult {
    let imported: Int
    let skipped: Int
    let failed: Int
    let sourceSchemaVersion: Int

    var total: Int { imported + skipped + failed }
}

private struct ExportPayload: Codable {
    let schemaVersion: Int?
    let minimumReadableSchemaVersion: Int?
    let format: String?
...
private enum ClipboardJSONImportError: LocalizedError {
    case newerSchema(Int)

    var errorDescription: String? {
        switch self {
        case .newerSchema(let version):
            return L("import.error.newerSchema", version)
        }
    }
}

private struct ExportItemDTO: Codable {
    let id: String?
    let type: String?
    let content: String?
    let preview: String?
    ...
    func makeClipboardItem() -> ClipboardItem? {
        guard let content,
              let typeRaw = type,
              let itemType = ClipboardItemType(rawValue: typeRaw) else {
            return nil
        }

        let decodedImageData = imageDataBase64.flatMap { Data(base64Encoded: $0) }
        let normalizedImagePayload: ImagePayloadStore.Payload?
        if itemType == .image, let decodedImageData {
            normalizedImagePayload = ImagePayloadStore.normalizedPayload(
                from: decodedImageData,
                preferredUTI: imageUTI
            )
        } else {
            normalizedImagePayload = nil
        }
        let storedImageData = itemType == .image
            ? (normalizedImagePayload?.data ?? decodedImageData)
            : nil
        ...
        if let parsedID = id.flatMap(UUID.init(uuidString:)) {
            item.id = parsedID
        }
        ...
        if let groupIDs {
            item.setGroupIDs(groupIDs.compactMap(UUID.init(uuidString:)))
        }

        if let normalizedImagePayload {
            ImagePayloadStore.applyMetadata(from: normalizedImagePayload, to: item)
        } else if itemType == .image, let storedImageData {
            item.imageUTI = imageUTI
            item.imageByteCount = imageByteCount ?? storedImageData.count
            item.imagePixelWidth = imagePixelWidth
            item.imagePixelHeight = imagePixelHeight
