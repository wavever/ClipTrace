...
enum AppContainer {
    ...
    private static let bundleFolderName = "com.wavever.clipth"
    ...
    static let shared: ModelContainer = {
        do {
            ...
            NSLog(
                "Clipth: opening ModelContainer failed (%@) — quarantining store and rebuilding",
    ...
    private static func quarantineUnopenableStore() {
        ...
        for artifact in present {
            ...
            NSLog("Clipth: could not back up %@ — deleting so the store can rebuild", artifact.source.lastPathComponent)
    ...
    /// Escape hatch (`defaults write com.wavever.clipth skipLegacyStoreScan
    ...
    private static func recoverLegacyStoreIfNeeded() throws {
        ...
        NSLog(
            "Clipth recovered %d clipboard history rows from legacy SwiftData store %@ into %@",
            source.count,
            source.url.path,
            destination.path
        )
    ...
    private static func mergeLegacyStoresIfNeeded(into container: ModelContainer) {
        ...
        do {
            ...
            if totalImported > 0 {
                try context.save()
                NSLog("Clipth imported %d missing clipboard history rows from legacy stores", totalImported)
            }
        } catch {
            NSLog("Clipth legacy history merge failed: %@", String(describing: error))
        }
    }
    ...
    private static func legacyStoreCandidates(excluding destination: URL) -> [URL] {
        ...
        let sandboxSupport = containerRoot
