...
final class SyncService: ObservableObject {
    ...
    func syncNow() {
        ...
        syncTask = Task { @MainActor [weak self] in
            ...
            do {
                ...
                activity = .succeeded
                NotificationCenter.default.post(name: .clipthSyncDidComplete, object: nil)
            } catch is CancellationError {
                activity = .idle
            } catch {
                let message = error.localizedDescription
                activity = .failed(message)
                NotificationCenter.default.post(name: .clipthSyncDidFail, object: message)
            }
            syncTask = nil
        }
    }
    ...
    private func makeTransport() throws -> any SyncTransport {
        ...
        case .localFolder:
            ...
            return FileSyncTransport(
                rootURL: url.appendingPathComponent("Clipth Sync", isDirectory: true),
                securityScoped: true
            )
        case .s3:
    ...
    private var iCloudDriveRootURL: URL? {
        ...
        return cloudDrive
            .appendingPathComponent("Clipth", isDirectory: true)
...
extension Notification.Name {
    static let clipthSyncDidComplete = Notification.Name("ClipthSyncDidComplete")
    static let clipthSyncDidFail = Notification.Name("ClipthSyncDidFail")
}
