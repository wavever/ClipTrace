...
/// notations `rgb()/rgba()`, `hsl()/hsla()` and `hsb()/hsv()`, plus bare
...
enum ColorValueParser {
    static func color(from raw: String) -> Color? {
        ...
        return hexColor(s) ?? functionalColor(s) ?? bareRGBColor(s)
    }
    ...
    /// Design tools commonly copy RGB values without the `rgb(...)` wrapper.
    /// Requiring commas and in-range channels keeps ordinary numeric text from
    /// being interpreted as a color too eagerly.
    private static func bareRGBColor(_ s: String) -> Color? {
        let parts = s
            .split(separator: ",", omittingEmptySubsequences: false)
            .map { $0.trimmingCharacters(in: .whitespaces) }
        guard parts.count == 3 || parts.count == 4,
              parts.allSatisfy({ !$0.isEmpty }),
              let r = bareChannel(parts[0]),
              let g = bareChannel(parts[1]),
              let b = bareChannel(parts[2]) else { return nil }

        let opacity: Double
        if parts.count == 4 {
            guard let parsedOpacity = bareAlpha(parts[3]) else { return nil }
    ...
    private static func bareChannel(_ str: String) -> Double? {
        guard let value = Double(str), (0...255).contains(value) else { return nil }
        return value / 255
    }

    private static func bareAlpha(_ str: String) -> Double? {
        if str.hasSuffix("%") {
            guard let value = Double(str.dropLast()), (0...100).contains(value) else { return nil }
            return value / 100
        }
        guard let value = Double(str), (0...1).contains(value) else { return nil }
        return value
    }
